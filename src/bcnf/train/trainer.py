...
from bcnf.train import TrainerDataHandler, TrainerModelHandler, TrainerUtilities, TrainerLossHandler, TrainerScheduler
...
class Trainer():
    ...
    def training_pipeline(self) -> None:
        ...
        with wandb.init(project=self.project_name,
            ...
            model, dataset, loss_function, optimizer, scheduler = self._make(self.config, self.device)
    ...
    def _make(self,
        ...
        # Make the data
        X, y = self.data_handler.generate_data_for_training(config = self.config["data"])
        ...
        return model, X, y, loss_function, optimizer, scheduler
        ...
        for epoch in pbar:
            ...
            if scheduler is not None:
                if isinstance(scheduler, torch.optim.lr_scheduler.ReduceLROnPlateau):
                    scheduler.step(val_loss_rolling_avg)
                else:
                    scheduler.step()
            ...
            if self.loss_handler.val_loss_patience is not None and self.loss_handler.val_loss_rolling_avg is not None:
                if self.loss_handler.val_loss_tolerance_mode == "rel":
                    if self.loss_handler.val_loss_rolling_avg < best_val_loss * (1 - self.loss_handler.val_loss_tolerance):
