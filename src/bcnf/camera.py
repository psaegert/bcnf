...
def camera(ball_pos: np.ndarray = np.array([0, 0, 1.5]),
    ...
    if show_plot:
        ...
        vals = ax.hist2d(ph, th, bins=[ratio[0] * 10, ratio[1] * 10], range=[[-phi, phi], [-theta, theta]], density=True, cmap='hot')
        ax.set_xlabel('horizontal angle')
        ax.set_ylabel('vertical angle')
        plt.show()

    else:
        vals = np.histogram2d(ph, th, bins=[ratio[0] * 10, ratio[1] * 10], range=[[-phi, phi], [-theta, theta]], density=True)

    # reverse and transpose vals[0] to return the correct orientation of the image
    return np.flipud(vals[0].T)
